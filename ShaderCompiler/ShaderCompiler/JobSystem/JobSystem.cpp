...
void VectorPool::Delete(vengine::vector<JobNode*>* targetPtr)
{
	...
	if (currentCount >= arr->capacity)
	{
		...
		if (currentCount >= arr->capacity)
		{
			...
			vengine::vector<JobNode*>** newArray = new vengine::vector<JobNode*> * [newCapacity];
			memcpy(newArray, arr->objs, sizeof(vengine::vector<JobNode*>*) * arr->capacity);
			delete arr->objs;
			arr->objs = newArray;
...
vengine::vector<JobNode*>* VectorPool::New()
{
	Array* arr = unusedObjects + objectSwitcher;
	int64_t currentCount = --arr->count;
	vengine::vector<JobNode*>* t;
	if (currentCount >= 0)
	{
		t = (vengine::vector<JobNode*>*)arr->objs[currentCount];

	}
	else
	{
		t = new vengine::vector<JobNode*>;
		t->reserve(20);
	}
