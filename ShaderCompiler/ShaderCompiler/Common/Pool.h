...
private:
	...
	constexpr void AllocateMemory()
	{
		...
		capacity = (uint64_t)(capacity * 1.5) + 8;
	}
public:
...
{
private:
	ArrayList<T*, false> allocatedPool;
	ArrayList<T*, false> list[2];
	std::mutex mtx;
	bool switcher = false;
	uint32_t capacity;
	void ReserveList(ArrayList<T*, false>& vec)
	{
		T* t = new T[capacity];
...
public:
	...
	T* New()
	{
		ArrayList<T*, false>& lst = list[switcher];
