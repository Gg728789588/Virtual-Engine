#include "MObject.h"
std::atomic<uint32_t> MObject::CurrentID = 0;
ArrayList<LinkHeap*> LinkHeap::heapPtrs;
...
void MObject::AddEventBeforeDispose(const Runnable<void(MObject*)>& func)
{
	if (!disposeFunc) disposeFunc = func;
	else
	{
		if (disposeFuncs.capacity() == 0) disposeFuncs.reserve(20);
...
PtrLink::PtrLink(const PtrLink& p) noexcept
{
...
PtrWeakLink::PtrWeakLink(const PtrLink& p) noexcept
{
	if (p.heapPtr && p.heapPtr->ptr) {
		++p.heapPtr->looseRefCount;
		heapPtr = p.heapPtr;
...
PtrWeakLink::PtrWeakLink(const PtrWeakLink& p) noexcept
{
	if (p.heapPtr && p.heapPtr->ptr) {
		++p.heapPtr->looseRefCount;
		heapPtr = p.heapPtr;
