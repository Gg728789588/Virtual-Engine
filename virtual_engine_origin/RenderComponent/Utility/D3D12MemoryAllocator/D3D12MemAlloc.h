...
*/
class __declspec(dllimport) Allocation
{
public:
...
*/
class __declspec(dllimport) Pool
{
public:
...
private:
    friend __declspec(dllimport) HRESULT CreateAllocator(const ALLOCATOR_DESC*, Allocator**);
    template<typename T> friend void D3D12MA_DELETE(const ALLOCATION_CALLBACKS&, T*);
    friend class Pool;
...
*/
__declspec(dllimport) HRESULT CreateAllocator(const ALLOCATOR_DESC* pDesc, Allocator** ppAllocator);
