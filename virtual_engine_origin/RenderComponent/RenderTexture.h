...
public:
	...
	inline static const D3D12_RESOURCE_STATES CommonState = D3D12_RESOURCE_STATE_COMMON;
	...
	void SetName(std::wstring const& str);
	~RenderTexture();
	...
	void BindRTVToHeap(DescriptorHeap* targetHeap, UINT index, ID3D12Device* device, UINT slice, UINT mip)const;
	...
	void BindUAVToHeap(DescriptorHeap* targetHeap, UINT index, ID3D12Device* device, UINT targetMipLevel)const;
